...
# This script updates holidays.json with 10 new destinations
# It downloads the current file, adds new destinations, and commits

echo "🌍 Adding 10 New Destinations to holidays.json"
echo "=============================================="

# Download current holidays.json from main branch
echo "📥 Downloading current holidays.json..."
...
if [ ! -f current_holidays.json ]; then
    echo "❌ Failed to download holidays.json"
    exit 1
fi

echo "✅ Downloaded successfully"
echo "📝 Current file size: $(wc -c < current_holidays.json) bytes"

# Create Python script to merge
cat > merge.py << 'PYTHON_EOF'
import json
...
print(f"Current destinations: {len(holidays)}")

# New destinations with AI-generated images
new_destinations = [
    {
...
holidays.extend(new_destinations)

print(f"New total: {len(holidays)} destinations")
...
print("✅ Updated file created: updated_holidays.json")
...
if [ -f updated_holidays.json ]; then
    echo "✅ Merge successful!"
    echo "📊 New file size: $(wc -c < updated_holidays.json) bytes"
    echo ""
    echo "📝 Summary:"
    echo "   - File ready: updated_holidays.json"
    echo "   - Next step: Copy this to src/data/holidays.json"
else
    echo "❌ Merge failed"
    exit 1
fi
...
echo ""
echo "=============================================="
echo "✨ Done! Ready to commit updated_holidays.json"
