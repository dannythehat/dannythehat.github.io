#!/usr/bin/env python3
"""
Script to add 10 new destinations with AI-generated images to holidays.json
Run this to update the holidays data file with new destinations
"""

import json
import sys

# All 10 new destinations with AI-generated images
NEW_DESTINATIONS = """
[
  {
...
def main():
    print("=" * 60)
    print("Adding 10 New Destinations to holidays.json")
    print("=" * 60)
    
    # Parse new destinations
    new_dests = json.loads(NEW_DESTINATIONS)
    
    print(f"\n✓ Loaded {len(new_dests)} new destinations")
    print("\nNew destinations:")
    for dest in new_dests:
        print(f"  - {dest['title']} ({dest['location']})")
    
    print("\n" + "=" * 60)
    print("To apply these changes:")
    print("1. Download current holidays.json from main branch")
    print("2. Append these destinations to the array")
