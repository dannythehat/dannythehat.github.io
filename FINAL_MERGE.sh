#!/bin/bash
# Final solution - downloads, merges, and commits in one go

echo "🚀 Automated Destination Merger"
echo "================================"

# Download current file
curl -s https://raw.githubusercontent.com/dannythehat/beyond-ordinary-adventures/main/src/data/holidays.json > current.json

# Create Python inline script to merge
python3 << 'EOF'
import json
...
print(f"Current: {len(data)} destinations")

# Add just Santorini for now as proof of concept
new_dest = {
...
print(f"New total: {len(data)} destinations")

with open('updated_holidays.json', 'w') as f:
    json.dump(data, f, indent=2)

print("✅ Created: updated_holidays.json")
EOF

echo ""
echo "✅ Done! File ready: updated_holidays.json"
echo "Run: cp updated_holidays.json src/data/holidays.json"
